...
class SmartSearchQA:
    ...
    def get_answer_from_conversational(self,query,
        ...
        retriever = self.get_retriever(vector_store,top_k,search_method,txt_docs_num,vec_docs_score_thresholds,txt_docs_score_thresholds,text_field,vector_field)
    ...
    def get_answer_from_multimodel(self,query,
        ...
        if len(rewrite_system_prompt) > 0:
            ...
            if self.aos_index.find('mobile') < 0 and self.aos_index.find('sales_brochures') < 0:
                query = rewrite_query
        ...
        if query.find('你的問題涉及不當用詞') >= 0:
            ...
            if module == 'RAG' and isCheckedKnowledgeBase:
                ...
                print('fix index:',index)
                
                vector_store = init_vector_store(self.embeddings,
                                                index,
                                                self.aos_host,
                                                self.aos_port,
                                                self.aos_username,
                ...
                retriever = self.get_retriever(vector_store,top_k,search_method,txt_docs_num,vec_docs_score_thresholds,txt_docs_score_thresholds,text_field,vector_field,image_field,work_mode,reranker_endpoint)
